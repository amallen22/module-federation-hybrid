...
const federationPlugin = !isDevelopment ? federation({
  ...
  remotes: useProductionUrls
    ...
    : {
      ...
      migrationPlan: 'http://localhost:5006/dist/assets/remoteEntry.js'
    },
  exposes: {
    './App': './src/App.tsx',
  },
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '@apps/migration-plan': resolve(__dirname, '../../apps/migration-plan/src'),
    }
  },
  server: {
    port: 5000,
    ...
    host: true,
    cors: {
      origin: ['http://localhost:5000', 'http://localhost:5001', 'http://localhost:5002', 'http://localhost:5003', 'http://localhost:5004', 'http://localhost:5006'],
      credentials: true
    }
  }
})
